...
class DeepLinkService {
  ...
  DeepLinkService({
    required DeepLinkClient deepLinkClient,
       ...
       deepLinkSubject = BehaviorSubject<Uri>() {
    unawaited(_getInitialLink());
    deepLinkSubscription = _deepLinkClient.deepLinkStream.listen(_onAppLink)
  ...
  @visibleForTesting
  /// A subject that emits URIs intercepted by the app.
  final BehaviorSubject<Uri> deepLinkSubject;

  @visibleForTesting
  /// A subscription to the deep link stream.
  ...
  Stream<Uri> get deepLinkStream => deepLinkSubject;
  ...
  void _onAppLink(Uri deepLink) {
    deepLinkSubject.add(deepLink);
  }

  void _handleError(Object error, StackTrace stackTrace) {
    deepLinkSubject.addError(DeepLinkClientFailure(error), stackTrace);
  }
